...
def load_vectors_shaped(vecFile, selector):
    ...
    for v in vector_names:
        ...
        vectors = vectors.dropna(subset=[VECTIME, VECVALUE]).copy()

        vals = vectors[VECVALUE].tolist()
        if vals and all(float(x).is_integer() for x in vals):
        ...
        if result.empty:
            ...
            result = merge(result, vectors, left_on=merge_columns, right_on=merge_columns)
    ...
    if MODULE not in result.columns:
        raise RuntimeError(f"Expected '{MODULE}' column before nodeId extraction, got columns={list(result.columns)}")

    node_ids = result[MODULE].astype(str).str.extract(r'actors\[(\d+)\]')[0]
    if node_ids.isna().any():
        bad = result.loc[node_ids.isna(), MODULE].drop_duplicates().tolist()
        raise RuntimeError(f"Could not extract nodeId from module path(s): {bad}")

    result["nodeId"] = node_ids.astype(int)
    ...
    present = [c for c in preferred if c in result.columns]
    extra = [c for c in result.columns if c not in present and c != MODULE]
    result = result[present + extra + [MODULE]]
